...
import 'package:lms_flutter_app/Model/CourseReport.dart';
...
// network call for Popular course list
  ...
  static Future<List<MyCourseModel>?> fetchMyCourse(String token) async {
    Uri myCourseUrl = Uri.parse('https://test.progemadphy.com/api/my-courses');
...
// user login
  static Future<Map<String, dynamic>?> login(
    ...
    try {
      Uri loginUrl = Uri.parse(
          'https://test.progemadphy.com/api/login'); // Updated endpoint
      Map data = {
  ...
  static Future register(name, email, phone, password, confirmPassword,
      studentWhatsAppNumber, guardianNumber) async {
    try {
      ...
      Map data = {
        ...
        "whatsapp_number": studentWhatsAppNumber.toString(),
        "guardian_number": guardianNumber.toString(),
      };

      //encode Map to JSON
      var body = json.encode(data);
      print('Registration request body: $body');
      print('Registration URL: $signUpUrl');
      print('Registration headers: ${header()}');
      ...
      if (response.statusCode == 200 || response.statusCode == 201) {
        ...
        if (jsonString['success'] == true || jsonString['data'] != null) {
          ...
          // Registration failed but got 200 response
          var errorMessage =
              jsonString['message']?.toString() ?? 'Registration failed';
          Get.snackbar(
            "Registration Failed",
            errorMessage,
            snackPosition: SnackPosition.BOTTOM,
  ...
  static Future<CourseMain?> getMyCourseDetails(int id) async {
    Uri topCatUrl =
        Uri.parse('https://test.progemadphy.com/api/get-course-details/$id');
  ...
  static Future<QuizStartModel?> startQuiz(
    ...
    Uri url = Uri.parse('https://test.progemadphy.com/quizTestStart');
  ...
  static Future<bool> singleAnswerSubmit({String? token, Map? data}) async {
    // Use the exact same URL as website
    Uri url = Uri.parse('https://test.progemadphy.com/singleQuizSubmit');
  ...
  static Future<QuizResultModel?> questionResult(
    ...
    // Use the exact same URL as website
    Uri url = Uri.parse(
        'https://test.progemadphy.com/quizResultPreviewApi/$quizResultId');
  ...
  static Future<bool?> finalQuizSubmit(int quizStartId) async {
    ...
    // Use the exact same URL as website
    Uri url = Uri.parse('https://test.progemadphy.com/quizSubmit');
  ...
  static Future<CourseReport?> getCourseReport(String token) async {
    try {
      Uri courseReportUrl =
          Uri.parse('https://test.progemadphy.com/api/course-report');
