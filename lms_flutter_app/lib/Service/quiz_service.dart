...
import 'package:lms_flutter_app/Model/Quiz/Quiz.dart';
...
class QuizService {
  ...
  static Future<List<MyQuizModel>?> getStudentQuizzes(String token) async {
    try {
      Uri url = Uri.parse(baseUrl + '/my-quizzes');

      var response = await http.get(url, headers: header(token: token));

      if (response.statusCode == 200) {
        var jsonString = jsonDecode(response.body);
        var courseData = jsonEncode(jsonString['data']);
        return List<MyQuizModel>.from(
                json.decode(courseData).map((x) => MyQuizModel.fromJson(x)))
            .toList();
      } else {
        debugPrint(
  ...
  static Future<CourseMain?> getCourseDetails(int courseId) async {
    try {
      ...
      Uri url = Uri.parse(
          'https://test.progemadphy.com/api/get-course-details/$courseId');
  ...
  static Future<QuizStartModel?> startQuiz({
    ...
    try {
      // Use exact URL format as Postman example
      Uri url = Uri.parse(
          'https://test.progemadphy.com/api/quiz-start/$courseId/$quizId');
  ...
  static Future<bool> submitSingleQuestion({
    ...
    try {
      // Use exact URL format as Postman example
      Uri url =
          Uri.parse('https://test.progemadphy.com/api/quiz-single-submit');
  ...
  static Future<bool> submitFinalQuiz({
    ...
    try {
      // Use exact URL format as Postman example
      Uri url = Uri.parse('https://test.progemadphy.com/api/quiz-final-submit');
  ...
  static Future<MyQuizResultsModel?> getQuizResult({
    ...
    try {
      // Use exact URL format as Postman example
      Uri url = Uri.parse(
          'https://test.progemadphy.com/api/quiz-result/$courseId/$quizId');
  ...
  static Future<Quiz?> getLessonQuizDetailsByQuizId(int quizId,
    ...
    try {
      ...
      // Use exact URL format as Postman example
      Uri url = Uri.parse(
          'https://test.progemadphy.com/api/get-lesson-quiz-details/$quizId');
